...
const TasksCompletedCarousel = () => {
    ...
    const getPastTasks = async () => {
        await axios.get(url + 'me/getMyCompletedTasks',
            ...
            >
                <Card style={{justifyContent: 'center', alignItems: 'center', width: 300, height: 200}} color="primary">
                    ...
                    {item.task.description? 
                        <CardSubtitle tag="h6" style={{width: 215}}><RiStickyNoteFill /> {
                            item.task.description.length < 57 ? item.task.description : item.task.description.substring(0,57) + "..."}
                        </CardSubtitle>
                        : 
                        <div></div>
                    }
    ...
    return (
        tasks.length > 0 ?
            ...
            :
            <div><h2>No Tasks Completed Yet</h2></div>
    )
}

export default TasksCompletedCarousel